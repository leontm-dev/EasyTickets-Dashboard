...
window.onload = () => {
    ...
    if (localStorage.getItem("EasyTickets-User") != null) {
        ...
        fetch(`${MAIN}/api/servers/${ID.replace("/prebuilds", "")}`, {
            method: "GET",
        })
        .then(res => {
            if (res.status == 200) {
...
document.getElementById("menu-item-tickets").addEventListener("click", (ev) => {
    window.location.href = `${MAIN}/dashboard/servers/id=${ID.replace("/prebuilds", "")}/tickets`;
});
document.getElementById("menu-item-new").addEventListener("click", (ev) => {
    window.location.href = `${MAIN}/dashboard/servers/id=${ID.replace("/prebuilds", "")}/new`;
});
document.getElementById("menu-item-prebuilds").addEventListener("click", (ev) => {
    window.location.href = `${MAIN}/dashboard/servers/id=${ID.replace("/prebuilds", "")}/prebuilds`;
});
